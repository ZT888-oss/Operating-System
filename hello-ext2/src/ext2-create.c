...
void write_superblock(int fd) {
    ...
    superblock.s_first_data_block  = 1;    // ext2 for 1024B blocks
    superblock.s_log_block_size    = 0;    // 1024 bytes
    superblock.s_log_frag_size     = 0;    // 1024 bytes
    superblock.s_blocks_per_group  =  8192;
    superblock.s_frags_per_group   =  8192;
    superblock.s_inodes_per_group  = NUM_INODES;
    superblock.s_mtime             = current_time;
    superblock.s_wtime             = current_time;
    superblock.s_mnt_count         = 0;
    superblock.s_max_mnt_count     = -1;   // unlimited
    superblock.s_magic             = 0xEF53;
    superblock.s_state             = 1;    // clean
    superblock.s_errors            = 1;    // continue on error
    superblock.s_minor_rev_level   = 0;
    superblock.s_lastcheck         = current_time;
    superblock.s_checkinterval     = 1;    // force check every 0 seconds
...
void write_block_bitmap(int fd) {
	...
	// Mark used blocks: 0 to LAST_BLOCK
...
void write_inode_bitmap(int fd) {
	...
	// Mark used inodes: 1 to LAST_INO
...
void write_inode_table(int fd) {
	...
	hello_inode.i_blocks = 0; // no data block used
...
void write_root_dir_block(int fd) {
    ...
    // .
    dir_entry_set(entry, EXT2_ROOT_INO, ".");
    dir_entry_write(entry, fd);
    bytes_remaining -= entry.rec_len;
    ...
    // lost+found
    dir_entry_set(entry, LOST_AND_FOUND_INO, "lost+found");
    dir_entry_write(entry, fd);
    bytes_remaining -= entry.rec_len;

    // hello-world
    dir_entry_set(entry, HELLO_WORLD_INO, "hello-world");
    dir_entry_write(entry, fd);
    bytes_remaining -= entry.rec_len;

    // hello (last entry)
    struct ext2_dir_entry last_entry = {0};
	dir_entry_set(last_entry, HELLO_INO, "hello");  // IMPORTANT
	last_entry.rec_len = bytes_remaining;           // fill remaining block
	dir_entry_write(last_entry, fd);
}
